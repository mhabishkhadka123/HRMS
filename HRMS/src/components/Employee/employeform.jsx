...
export default function EmployeeForm() {
  setEmployees, setModelForm, editEmployee, setEditEmployees;
  ...
  useEffect(() => {
    if (editEmployee) {
      ...
      setSalary(esditEmployee.salary);
    }
  }, [editEmployee]);
  const handleSubmit = async (e) => {
    ...
    const employeeData = {
      ...
      salary,
      password,
      date: new Date().toLocaleDateString().split("T")[0], //today's date
    };
    ...
    try {
      if (editEmployee) {
        ...
        const response = await axios.post(
          ...
          {
            headers: {
              "Content-Type": "application/json", // to check the token
              Authorization:`Bearer ${token}`,
            },
          }
        );
        setEditEmployee((prev) => [...prev, response.data]);
        setModelForm(false);
      ...
      setName(""); // makes the fields empty for other form filling
      setEmail("");
      setDepartment("");
      setDesignation("");
      setUserType("");
